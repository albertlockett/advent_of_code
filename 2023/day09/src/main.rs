...
fn test_lagrange_num_expand() {
    let mut lagrange_num = LagrangeNum {
        coefficients: vec![1],
    };
    // y = (x - 1) = -1 + x
...
fn test_input() {
    ...
    polynomial.expand(nums[0]);
    polynomial.expand(nums[1]);
    polynomial.expand(nums[2]);

    println!("2. x = 0, y = {}", polynomial.eval(0));
    println!("2. x = 1, y = {}", polynomial.eval(1));
    println!("2. x = 2, y = {}", polynomial.eval(2));
    println!("2. x = 3, y = {}", polynomial.eval(3));
}
