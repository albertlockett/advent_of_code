...
fn main() {
    ...
    for line in strat_guide_encrypted.split("\n") {
        ...
        println!(
            "P1: for enemy={:?} me={:?} i scored {:?}",
            en_play_encrypted, me_play_encrypted, p1_pts
        );
        println!(
            "P2: for enemy={:?} me={:?} i scored {:?}",
            en_play_encrypted, me_play_encrypted, p2_pts
        );
    ...
    println!(
        "PART 1 = at the end of the elf RPS tournament I won {:?}",
        p1_total_points
    );
    println!(
        "PART 2 = using the real docding I won this many pts {:?}",
        p2_total_points
    );
}
