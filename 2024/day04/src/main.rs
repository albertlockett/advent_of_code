...
async fn main() {
    ...
    // let batch =
    //     RecordBatch::try_new(schema.clone(), vec![Arc::new(list_builder.finish())]).unwrap();
    // ctx.register_batch("grid_t_rev", batch).unwrap();
    ...
    let view_defs = vec![
        ...
        // format!("create view cols_tmp_rev as {} from grid_t", tmp_view_def),
        format!(
        ...
        // ),
        // format!(
        //     "create view grid_t_coords_rev as {} from cols_tmp_rev order by y",
        //     coords_view_def
        // ),
        ...
        // "create view diag2_rev as select array_agg(s order by y desc) as s, x + y as sum from grid_t_coords_rev group by x + y order by x + y"
        //     .to_string(),
    ...
    // let result = ctx.sql(
    //     // "select sum(c) from matches",
    //     "select * from diag2_rev"
    // ).await.unwrap().collect().await.unwrap();
    // print_batches(&result).unwrap();

    // println!("part 1 = {:?}", result); // WHY DOESNT THIS WORK!!!!!!
